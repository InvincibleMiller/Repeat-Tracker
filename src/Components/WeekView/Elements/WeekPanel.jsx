...
const WeekPanel = ({ week }) => {
  ...
  const getCompletionTables = () => {
    ...
    const scoreFAM = getLabelCheckCompletionRate(
      fAM,
      labelChecksExpectedDuringWeekPerShift
    );
    const scoreFPM = getLabelCheckCompletionRate(
      fPM,
      labelChecksExpectedDuringWeekPerShift
    );
    const scoreKAM = getLabelCheckCompletionRate(
      kAM,
      labelChecksExpectedDuringWeekPerShift
    );
    const scoreKPM = getLabelCheckCompletionRate(
      kPM,
      labelChecksExpectedDuringWeekPerShift
    );
      ...
      <>
        <div className="sub-panel">
          <div className="sub-panel-2">
            <h2 className="shift-heading mb-2 text-lg">
  ...
  return (
    <div
      className={`panel w-full origin-top ${
        weeklyData ? "scale-y-100" : "scale-y-0"
      }
                         transition-all duration-[0.4s]`}
    >
      <div className="heading mb-3 h-10 overflow-clip">
        {week && getDateRange(week)}
      </div>
      <div className="sub-panel mb-2">
        <h2 className="shift-heading">Results</h2>
        <div className="sub-panel-2">
          <h2 className="shift-heading text-lg">
            Score:{" "}
            {weeklyData && Math.floor(weeklyData.weeksResult.score * 100) / 100}
          </h2>
        </div>
      </div>
